...
def extract_augmentation_command(augment_str, df):
    """Extract augmentation command from augmentation string."""
    answer_start_str = "Final output:\n"
    start_ind = augment_str.find(answer_start_str)
    if start_ind == -1:
        out_str = augment_str
    else:
        out_str = augment_str[start_ind + len(answer_start_str):]
    commands = out_str.split("\n")
    results = []
    match_pat = re.compile(r"`(\w+)` = @\(\"(.+)\"; \[(.+)\]\)")

    for row in commands:
        res = match_pat.match(row)
        if res:
            new_col, question, exist_cols = res.groups()
            exist_cols = exist_cols.replace(", ", ",").split(",")
            exist_cols = [col.strip() for col in exist_cols]
            exist_cols = [col.strip("`") for col in exist_cols]
            # check if the exist_cols are in the table
            if all([col in df.columns for col in exist_cols]):
                # add `` around the column name
                exist_cols = ["`{}`".format(col) for col in exist_cols]
                results.append((new_col, question, exist_cols))
            else:
                match_cols = []
                for col in exist_cols:
                    if col in df.columns:
                        ...
                        # Use column with the highest fuzzy match score
                        scores = [fuzz.ratio(col, _col) for _col in df.columns]
                        match_cols.append(df.columns[np.argmax(scores)])
                match_cols = ["`{}`".format(col) for col in match_cols]
                results.append((new_col, question, match_cols))
    return results


def extract_added_table(augment_str):
    answer_start_str = "So the output is:" if "So the output is:" in augment_str else "Final output:"
    start_ind = augment_str.find(answer_start_str)
    if start_ind == -1:
        out_str = augment_str
    else:
...
def extract_sql_query(sql_str):
    """Extract sql query from output string."""
    
    start_idx, offset = -1, 0
    if start_idx == -1:
        start_idx = sql_str.find("Final SQL query:\n```")
        offset = len("Final SQL query:\n```")
    if start_idx == -1:
        start_idx = sql_str.find("Final SQL query:\n")
        offset = len("Final SQL query:\n")
    if start_idx == -1:
        start_idx = sql_str.find("Final output:\n```")
        offset = len("Final output:\n```")
    if start_idx == -1:
        start_idx = sql_str.find("Final output:\n")
        offset = len("Final output:\n")
    if start_idx == -1:
        start_idx = sql_str.find("SELECT")
        offset = 0
    end_idx = sql_str.find("```", start_idx + offset)
    if end_idx == -1:
        end_idx = sql_str.find("Units: ", start_idx)
    if end_idx == -1:
        end_idx = sql_str.find("So the unit is", start_idx)
    out_str = sql_str[start_idx + offset:end_idx]
    out_str = out_str.replace("sql:\n", "").replace("SQL:\n", "").replace("sql\n", "").replace("SQL\n", "").replace("```", "").strip()
    
    return out_str


def extract_units(sql_str):
    """Extract units from output string."""
    start_idx = -1
    if start_idx == -1:
        start_idx = sql_str.find("Units: ")
        offset = len("Units: ")
    if start_idx == -1:
        start_idx = sql_str.find(": ")
        offset = len(": ")
    if start_idx == -1:
        out_str = ""
    else:
        out_str = sql_str[start_idx + offset:]
        out_str = out_str.replace("```", "").replace(".", "").strip()
        try:
            out_str = eval(out_str)
        except:
            pass
...
def extract_binder_map(output_str):
    """Extract augmentation for Binder."""
    match_pat = r"QA\(\"map@(.+?\??)\"; (.+?)\)"
    matches = re.findall(match_pat, output_str)
    return matches


def extract_solution_formula(aug_str):
    """Extract solution formula from augmentation string."""
    start_idx = aug_str.find("Solution formula:")
    end_idx = aug_str.find("Further analysis:")
    if end_idx != -1:
        if start_idx == -1:
            return aug_str[:end_idx].strip()
        else:
            return aug_str[start_idx + len("Solution formula:"):end_idx].strip()
    
    return None


def extract_required_variable(augment_str):
    answer_start_str = "Further analysis:"
    start_ind = augment_str.find(answer_start_str)
    if start_ind == -1:
        out_str = augment_str
    else:
        out_str = augment_str[start_ind + len(answer_start_str):]
    answer_end_str = "Final output:"
    end_ind = out_str.find(answer_end_str)
    out_str = out_str[:end_ind].strip()
    return out_str
