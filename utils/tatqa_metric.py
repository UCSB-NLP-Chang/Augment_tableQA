...
def get_answer_str(answers: list, scale: str):
    ...
    for ans in answers:
        ...
        if is_number(ans_str):
            ...
            if scale:
                ans_str = ans_str + " " + str(scale)
        ans_temp.append(ans_str.lower())
    # sort answer to ensure the order
    ans_temp = sorted(ans_temp)
    ...
    def __call__(self,
        ...
        if not prediction:
            ...
            if not gold_answer:
                ...
                # if some prediction is a number but some is string, we need to convert them to the same type
                if any([pred is None for pred in prediction]) or \
                    (any([isinstance(pred, str) for pred in prediction]) and not all([isinstance(pred, str) for pred in prediction])):
