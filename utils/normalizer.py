...
def str_normalize(user_input, recognition_types=None, datasetname='wikitq'):
    ...
    if datasetname == 'tatqa':
        res = re.match(r"^\s*\$?\s*\(\s*(\d+(\.\d+)?)\s*\)\s*$", user_input)
        if res:
            user_input = "-" + res.group(1)
    elif datasetname == 'finqa':
        res = re.match(r"^\s*-?([\d\.]+%?)\s*\(\s*0?\1\s*\)\s*$", user_input)
        if res:
            user_input = res.group(1)
        if '%' in user_input:
            try:
                ...
                if '.' in clean_input:
                    precision = len(clean_input.split('.')[-1])
                else:
                    precision = 0
                val = round(val / 100, precision + 2)
                user_input = str(val)
            except:
                pass
...
def prepare_df_for_neuraldb_from_table(table: Dict, dataset_name: str, add_row_id=True, normalize=True, lower_case=True):
    ...
    if normalize:
        df = convert_df_type(pd.DataFrame(data=rows, columns=header), dataset_name, lower_case=lower_case)
    else:
...
def convert_df_type(df: pd.DataFrame, dataset_name: str, lower_case=True):
    """
    ...
    if dataset_name == 'finqa' or dataset_name == 'tatqa':
        recognition_types += ["currency", "percentage"]
    for header in df.columns:
        df[header] = df[header].map(lambda x: str_normalize(x, recognition_types, datasetname=dataset_name))
    ...
    for header in df.columns:
        ...
        if dataset_name == 'tatqa' or dataset_name == 'finqa':
            try_int = header == 'row_id' or header.startswith('year') or ('fiscal year' in header)
        else:
            try_int = True

        if float_able and try_int:
            try:
...
def post_process_sql(sql_str, df, table_title=None, process_program_with_fuzzy_match_on_db=True,
                     wrap_column=True, verbose=False):
    ...
    def basic_fix(sql_str, all_headers, table_title=None):
        ...
        # match headers
        ignore_set = set(string.ascii_letters + string.digits + "_`")
        for header in all_headers:
            if (header in sql_str) and (header not in ALL_KEY_WORDS):
                ...
                for matched_of_this_header in all_matched_of_this_header:
                    ...
                    # add the case where header is within some text
                    if all(have_matched[start_idx: end_idx]) == 0 and (sql_str[start_idx - 1] not in ignore_set) and (
                            sql_str[end_idx] not in ignore_set):
